...
public class GroupWorker {
    ...
    private static Set<String> departs = Sets.newHashSet();
    ...
    public static void main(String[] args) {
        ...
        // remainWorkers:25个人一组，总共538人，获得剩下的13个人再加进去
//        remain13Worker = getRemain13Worker();

        // departs：赛选出所有不同的部门,总104个部门
        departs = allWorkers.stream().map(Worker::getDepartment).collect(Collectors.toSet());
    ...
    public static List<Worker> getRemain13Worker() {
        List<Worker> res = Lists.newArrayList();
        // 25个人一组，总538人，有13人没法分组
        for (int i = 0; i < 13; i++) {
            Worker one = allWorkers.get((int) (Math.random() * allWorkers.size()));
            allWorkers.remove(one);
            res.add(one);
        }
        return res;
    }
